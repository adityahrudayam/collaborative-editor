...
module.exports = (req, res, next) => {
    ...
    User.findByPk(req.uid).then(user => {
        ...
        if (req.method === 'POST' && req.url.includes('/publish-document/')) {
        ...
        } else if (req.method === 'PUT' && req.url === '/edit-publisher/change-edit-status' && user.uid !== req.body?.owner)
            throw new Error("Unauthorized Request! Please check & try again.");
        else if (req.method === 'PUT' && req.url === '/edit-publisher/change-edit-status-by-username' && user.username !== req.body?.owner)
            throw new Error("Unauthorized Request! Please check & try again.");
        else if (req.method === 'PUT' && req.url === '/edit-publisher/change-edit-status-by-email' && user.email !== req.body?.owner)
            throw new Error("Unauthorized Request! Please check & try again.");
